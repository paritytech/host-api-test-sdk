...
- **`ChainSubmit` is requested by signing, not by connecting or allocating.** A suite that connects, allocates and then reads `getPermissionLog()` sees an empty log for that reason alone — the entry appears once the product signs. The `ResourceAllocation` confirmation that a product requesting allocation at connect does trigger is a separate decision, recorded in `getUserConfirmationLog()`; it is not `ChainSubmit` arriving by another route. Pinned by a test that asserts the permission log is empty at connect and after an allocation, and carries `ChainSubmit` after a signature.
